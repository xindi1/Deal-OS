...
function buildPressureItems() {
  ...
  const overdueComms = state.communications.filter(c => isOverdue(c.followUp));
  ...
  if (engaged < 3) items.push(`Only ${engaged} target(s) are in Engaged. Move at least 3 there so the loop creates real momentum.`);
...
function getSummaryStats() {
  ...
  const followUpsDue = state.communications.filter(c => isOverdue(c.followUp)).length;
  ...
  return {
    ...
    pipelineValue,
    followUpsDue,
    lastAction: sortedComms[0]?.timestamp || ''
  };
}
...
function renderHeroStats(stats) {
  ...
  strip.innerHTML = [
    ['Targets', stats.totalTargets],
    ['Touches', stats.outreachCount],
  ...
  document.getElementById('closeGoalLabel').textContent = `${stats.closedDeals} / 1 deal closed`;
  document.getElementById('closeGoalBar').style.width = `${Math.min(100, stats.closedDeals * 100)}%`;
  document.getElementById('lastActionBadge').textContent = stats.lastAction
    ? `Last action ${formatDateTime(stats.lastAction)}`
    : 'No actions yet';

  const todayLine = document.getElementById('todayLine');
  if (!stats.totalTargets) {
    todayLine.textContent = 'Start by creating the first live target.';
  } else if (stats.followUpsDue) {
    todayLine.textContent = `${stats.followUpsDue} overdue follow-up${stats.followUpsDue === 1 ? '' : 's'} — handle those first.`;
  } else if (!stats.outreachCount) {
    todayLine.textContent = 'You have targets loaded. Send the first touch.';
  } else {
    todayLine.textContent = `${stats.totalTargets} target${stats.totalTargets === 1 ? '' : 's'} in play · ${stats.engagedCount} beyond cold stage.`;
  }
}
...
function renderMetrics() {
  ...
  const metricData = [
    ['Targets', stats.totalTargets, stats.totalTargets ? 'Active list of people or entities in play' : 'Add your first target'],
    ['Outreach Logged', stats.outreachCount, stats.outreachCount ? 'Every outbound action increases pressure' : 'No outreach logged yet'],
    ['Engaged+', stats.engagedCount, 'Targets beyond simple identification'],
    ['Pipeline Value', formatCurrency(stats.pipelineValue), 'Estimated total value in play'],
    ['Closed Deals', stats.closedDeals, stats.closedDeals ? 'Progress is real' : 'Still hunting the first win'],
    ['Follow-Ups Overdue', stats.followUpsDue, stats.followUpsDue ? 'These need attention now' : 'No overdue follow-ups']
  ];
...
function sortedTargets() {
  return state.targets.slice().sort((a, b) => {
...
function renderTargetCards() {
  const list = document.getElementById('targetCards');
  const rows = sortedTargets();

  list.innerHTML = rows.length ? rows.map(target => `
    <article class="target-card">
      <div class="target-card-top">
        <div>
          <h3>${escapeHtml(target.name)}</h3>
          <p class="target-meta">${escapeHtml(target.organization || target.source)}</p>
        </div>
        <span class="badge">${escapeHtml(target.stage)}</span>
      </div>
      <div class="target-row-grid">
        <div class="target-mini">
          <span>Value</span>
          <strong>${target.value ? formatCurrency(target.value) : '—'}</strong>
        </div>
        <div class="target-mini">
          <span>Warmth</span>
          <strong>${escapeHtml(String(target.warmth))} / 5</strong>
        </div>
        <div class="target-mini">
          <span>Last Touch</span>
          <strong>${formatDateTime(target.lastTouch)}</strong>
        </div>
        <div class="target-mini">
          <span>Next Follow-Up</span>
          <strong>${formatDate(target.nextFollowUp)}</strong>
        </div>
      </div>
      <p class="target-meta">${escapeHtml(target.source)}${target.notes ? ` · ${escapeHtml(target.notes)}` : ''}</p>
      <div class="small-actions">
        <button type="button" class="secondary" onclick="editTarget('${target.id}')">Edit</button>
        <button type="button" class="danger" onclick="deleteTarget('${target.id}')">Delete</button>
      </div>
    </article>
  `).join('') : '<p class="muted">No targets yet.</p>';
}
...
function jumpToSection(hash, focusId) {
...
function toggleSettings() {
  document.getElementById('settingsSheet').classList.toggle('hidden');
}
...
function setupTabObserver() {
  ...
  const observer = new IntersectionObserver(entries => {
    ...
    links.forEach(link => link.classList.toggle('active', link === lookup.get(id)));
  }, {
...
const jumpTargetBtn = document.getElementById('jumpTargetBtn');
if (jumpTargetBtn) jumpTargetBtn.addEventListener('click', () => jumpToSection('#targets', 'targetName'));
const jumpCommBtn = document.getElementById('jumpCommBtn');
if (jumpCommBtn) jumpCommBtn.addEventListener('click', () => jumpToSection('#communications', 'commSummary'));
...
if (fabToggle) fabToggle.addEventListener('click', toggleQuickSheet);
const fabAddTarget = document.getElementById('fabAddTarget');
if (fabAddTarget) fabAddTarget.addEventListener('click', () => { closeQuickSheet(); jumpToSection('#targets', 'targetName'); });
const fabAddTouch = document.getElementById('fabAddTouch');
if (fabAddTouch) fabAddTouch.addEventListener('click', () => { closeQuickSheet(); jumpToSection('#communications', 'commSummary'); });

document.querySelectorAll('.tab-link').forEach(link => {
  link.addEventListener('click', () => closeSettings());
});

document.addEventListener('click', event => {
  const sheet = document.getElementById('settingsSheet');
  const toggle = document.getElementById('toggleSettingsBtn');
  const quickSheet = document.getElementById('quickActionSheet');
  const fab = document.getElementById('fabToggle');
  if (sheet && !sheet.classList.contains('hidden')) {
    if (!(sheet.contains(event.target) || toggle.contains(event.target))) closeSettings();
  }
  if (quickSheet && !quickSheet.classList.contains('hidden')) {
    if (!(quickSheet.contains(event.target) || fab.contains(event.target))) closeQuickSheet();
  }
});
...
window.editTarget = function editTarget(id) {
  ...
  jumpToSection('#targets', 'targetName');
};
...
window.editComm = function editComm(id) {
  ...
  jumpToSection('#communications', 'commSummary');
};
